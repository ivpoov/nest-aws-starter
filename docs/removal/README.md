...
- **s3 / sqs / sns / mail / lambda (providers)** — These providers predate the fence-marker convention and were never retrofitted. The per-provider coupling notes below say what each one is actually tied to; making them removable is a dedicated pass, not a side effect of another change.
