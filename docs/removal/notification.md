...
Notification/receipt/preference schema, WS gateway, the persist-first event subscriber (IN_APP + the per-type/per-channel EMAIL gate, PR 5), the history API (list/unread-count/mark-read/read-all), and the preferences API (GET/PUT matrix).
...
- `apps/api/test/notification-event-subscriber.e2e-spec.ts` (delete)
