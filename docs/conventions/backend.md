...
import { NoteResponseDto } from '@modules/note/dtos/responses/note-response.dto';
import { CreateNoteDto } from '@modules/note/dtos/create-note.dto';
...
@ApiBearerAuth()
@ApiTags('Notes')
@UseGuards(ThrottlerBehindProxyGuard, JwtAuthGuard, AccessGuard)
@Controller('notes')
export class NoteController {
  ...
  @UseAbility(Actions.create, NoteEntity)
  @Post()
  public create(@Body() dto: CreateNoteDto): Promise<NoteInterface> {
    return this.noteService.create(dto);
  }

  @ApiDefaultResponse({ status: StatusCodes.OK, type: NoteResponseDto, isArray: true })
  @Serialize(NoteResponseDto)
  @UseAbility(Actions.read, NoteEntity)
  @Get()
  public findMany(@Query() query: PaginationQueryDto): Promise<NoteInterface[]> {
    return this.noteService.findMany(query);
  }

  @ApiDefaultResponse({ status: StatusCodes.NO_CONTENT })
  @UseAbility(Actions.delete, NoteEntity)
...
import { NoteService } from '@modules/note/services/note.service';
import { NotePrismaRepository } from '@modules/note/repositories/note-prisma.repository';
import { NOTE_REPOSITORY } from '@modules/note/constants/note.constants';
