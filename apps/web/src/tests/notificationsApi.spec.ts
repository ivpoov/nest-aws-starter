...
describe('fetchNotifications', () => {
  ...
  it('serializes cursor, limit, unreadOnly, type and audience together', async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith(
      '/notifications?cursor=0198c0de-0000-7000-8000-000000000001&limit=10&unreadOnly=true&type=PASSWORD_CHANGED&audience=USER',
    );
  });

  it('omits unreadOnly entirely when it is false rather than sending unreadOnly=false', async () => {
    await fetchNotifications({ limit: 10, unreadOnly: false });
