...
// notification-events.constants.ts for why the `unread-count` push is
// ignored) — it comes from GET /notifications/unread-count, fetched once on
// auth and then on a fixed poll, so the badge keeps moving even when the
// socket is down entirely. The socket still drives live UX: a `notification`
// arrival bumps the badge by one immediately (see notificationSocketReducer)
// so it does not sit stale for a full poll interval.
