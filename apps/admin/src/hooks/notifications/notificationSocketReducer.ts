...
export function notificationSocketReducer(
  ...
  switch (action.kind) {
    ...
    case 'notification-received':
      // Also bumps the badge by one: unlike web, the admin bell does not
      // trust the gateway's `unread-count` push for its merged USER+ADMIN
      // figure (see notification-events.constants.ts), so a live arrival's
      // only immediate signal is this optimistic +1 — the next poll tick
      // (or the mark-read refetch) reconciles it against the server.
      return {
        ...state,
