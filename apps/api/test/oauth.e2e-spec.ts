...
describe('oauth flow (fake provider)', () => {
  ...
  // suite's `WEB_APP_BASE_URL=http://localhost:20001` the suffix lands inside
  ...
  // the `localhost:200010` row, which reaches the origin comparison.
  it.each([
    [
      'a host that only suffixes the allowed origin',
      'http://localhost:20001.evil.tld/cb',
      'BAD_REQUEST',
    ],
    [
      // 2000 is a prefix of the allowed 20001. The old fixture appended a digit
      // instead (5173 -> 51730), which cannot be expressed against a five-digit
      // port: 200010 is above 65535 and fails as a malformed URL long before the
      // allowlist is consulted, which tests the parser rather than the check.
    ...
    [
      ...
      'http://localhost:20001/settings/methods',
      'OAUTH_REDIRECT_NOT_ALLOWED',
    ],
