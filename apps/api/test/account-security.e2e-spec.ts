...
describe('account security', () => {
  ...
  it('rejects unauthenticated and non-admin callers', async () => {
    await request(app.getHttpServer()).get('/api/v1/admin/account-security/lockouts').expect(401);

    const forbidden = await request(app.getHttpServer())
      .get('/api/v1/admin/account-security/lockouts')
    ...
    await request(app.getHttpServer())
      .delete('/api/v1/admin/account-security/lockouts/anything')
      .set('authorization', `Bearer ${userToken}`)
      .expect(403);
  });
  ...
  describe('per-email lockout', () => {
    it('locks the account after 5 failed attempts from 5 different ips, admin can list and release it', async () => {
      ...
      await request(app.getHttpServer())
        .delete(`/api/v1/admin/account-security/lockouts/${entry.key}`)
        .set('authorization', `Bearer ${adminToken}`)
        .expect(204);
